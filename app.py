...
def ejecutar_impresion(html_content):
    ...
    component_script = f"""
    <div id="ticket-{unique_id}" style="display:none;">
        {html_content}
    </div>
        ...
        <table style="width:100%; font-size:10px;">
            {
                "".join([
                    ...
                    <tr>
                        ...
                        <td align='right'>${it['subtotal']:,.2f}</td>
                    </tr>
                    """
                    for it in items
                ])
...
if 'ticket_a_imprimir' not in st.session_state:
...
# =========================================================
    ...
    df_inv = get_df(
        "SELECT * FROM inventario WHERE stock > 0"
    )
    ...
    if not df_inv.empty:
        ...
        with c2:
            ...
            if st.session_state.carrito:
                ...
                if st.button(
                    f"✅ Finalizar e Imprimir (${total_v:,.2f})",
                    type="primary",
                    use_container_width=True
                ):
                    ...
                    st.session_state.ticket_a_imprimir = generar_ticket_html(
                        "TICKET VENTA",
                        t_id,
                        st.session_state.carrito,
                        total_v
...
# =========================================================
    ...
    with st.form("ap_f"):
        ...
        sel = st.selectbox(
            "Prenda",
            df_inv['lbl'] if not df_inv.empty else ["Vacío"]
        )
...
# =========================================================
    ...
    if not df_corte.empty:
        ...
        m3.metric(
            ...
            f"${utilidad:,.2f}",
            delta=(
                f"{(utilidad/total_ventas)*100:.1f}%"
        ...
        st.info(
            f"No hay ventas registradas "
            f"desde el {fecha_inicio}"
        )
...
# =========================================================
    ...
    st.header("Administrador de Inventario")
    ...
    st.subheader("📂 Importar Inventario Masivo")
        ...
        try:
            ...
            if faltantes:
                ...
                if st.button(
                    ...
                    st.success(
                        f"✅ {len(df_excel)} registros importados correctamente."
                    )
    ...
    if os.path.exists(DB_NAME):
        ...
        with open(DB_NAME, "rb") as f:
            ...
            st.download_button(
                "📥 Descargar DB",
                f,
                f"Backup_Ilusion_{datetime.now().strftime('%Y%m%d')}.db"
            )
